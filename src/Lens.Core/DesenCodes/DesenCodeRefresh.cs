...
///   • Art arda cok fazla baglanti hatasinda islem DURUR.
...
public static class DesenCodeRefresh
{
        ...
        ILensLogger? logger = null)
    {
        ...
        foreach (var group in groups)
        {
            ...
            if (consecutiveFailures >= Math.Max(1, DesenCodeServiceOptionsDefaults.AbortThreshold))
            {
        ...
        return new DesenCodeRefreshResult(
            outcome, relativePaths.Count, groups.Count, updated, notFound, failed, null);
    }
...
/// <summary>Orkestrasyonun servis nesnesinden bagimsiz calisabilmesi icin esik degeri (test edilebilirlik).</summary>
internal static class DesenCodeServiceOptionsDefaults
{
    internal const int AbortThreshold = 5;
}
